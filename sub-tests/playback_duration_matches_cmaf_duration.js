function playback_duration_matches_cmaf_duration() {
  ...
  var t = {
    ...
    current_time: async_test(
      "CurrentTime when ended is equal to duration in MPD"
    ),
  };
        ...
        if (numberOfAppendedVideoSegment >= segmentsNumber) {
          ...
          t.video_duration.step(function () {
            assert_equals(
              ...
              player.getCurrentManifest().duration
            );
          });
  ...
  player.registerCallbacks("onSourceBufferAdded", handleSourceBufferAdded);
