function DpctfTest(config) {
  ...
  function setupTest(error) {
    ...
    return new Promise(function (resolve) {
      ...
      player.on(Player.PLAYER_EVENT_PLAYBACK_RATE_CHANGE, function (rate) {
        logger.info("playback rate change: " + rate);
      });
      ...
      player
        .init({
          ...
          mseResetTolerance: parameters.mseResetTolerance,
  ...
  function finishTest() {
    return (logBuffer ? logBuffer.flush() : Promise.resolve()).then(function () {
      done();
    });
  }
  ...
  function updateQrCode(currentTime) {
    ...
    if (player) object.pr = player.getPlaybackRate();
  ...
  function fetchParameters() {
    return new Promise(function (resolve) {
      ...
      xhr.addEventListener("load", function () {
        ...
        resolve(buildParameters(testConfig, testInfo));
      });
...
function LogBuffer(logger, testPath, token) {
  ...
  function flush() {
    if (bufferedLogs.length === 0) return Promise.resolve();
    var logs = bufferedLogs.slice();
    bufferedLogs = [];
    return WaveService.sendLogs(token, testPath, logs);
  }
...
function buildParameters(testConfig, testInfo) {
  ...
  var parameters = {
    ...
    framePresentedTolerance: determineValue("frame_presented_tolerance"),
    mseResetTolerance: determineValue("mse_reset_tolerance"),
  };
