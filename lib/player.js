...
function BufferManager(manifests, mediaSource, video, options) {
  ...
  function setSegments(segmentsInfo) {
    ...
    _mediaSource.duration = totalDuration;
  ...
  function setGaps(gaps) {
    ...
    for (var gap of gaps) {
      _gaps.push({
        fragmentNumber: gap.gapLocationFragment,
  ...
  function appendQueuedBuffers() {
    ...
    for (var gap of _gaps) {
      var startSegment = gap.fragmentNumber;
      var endSegment;
      var duration = 0;
      for (var index in _segments) {
	if (index < startSegment) continue;
        duration += _segments[index].getDuration();
        if (gap.gapDuration > duration) continue;
	endSegment = index;
	break;
      }
      console.log("start", startSegment, "end", endSegment);
      var segmentNumber = bufferInfo.segment.getNumber();
      if (segmentNumber >= startSegment && segmentNumber <= endSegment) {
	console.log("dropping segment", segmentNumber);
        return;
      }
    }
