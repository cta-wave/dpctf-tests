...
function Player(video) {
  ...
  function loadVideo(vectorUrls) {
    ...
    return Promise.all(
      vectorUrls.map((vectorUrl, index) => ManifestParser.parse(vectorUrl, index))
      ...
      for (let manifest of manifests) {
        ...
        for (var periodId in manifest.getPeriods()) {
          ...
          if (!_duration) setDuration(duration);
...
function BufferManager(manifests, mediaSource, video, options) {
  ...
  function setPlayingRepresentation(representation) {
    if (
      ...
      _playingRepresentation.getManifestIndex() === representation.getManifestIndex()
    )
      return;
