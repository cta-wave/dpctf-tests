...
function Player(video, options) {
  ...
  let _mseResetBoundaries = {};
  let _mseResetPending = false;
  let _reinitializing = false;
  let _resetBoundary = null;
  let _resetSafetyTimeout = null;
  ...
  function init(settings) {
    ...
    _eventEmitter.on(
      MEDIA_SOURCE_SHOULD_REINITIALIZE,
      function (payload) {
        requestMediaSourceReinitialization(payload);
      },
    );
  ...
  function loadMedia({ vectorUrls, mediaType }) {
    ...
    return Promise.all(
      ...
      bufferManager.on(MEDIA_SOURCE_SHOULD_REINITIALIZE, function (payload) {
        _eventEmitter.dispatchEvent(MEDIA_SOURCE_SHOULD_REINITIALIZE, payload);
      });
      var promises = [];
  ...
  function requestMediaSourceReinitialization(payload) {
    if (!payload) return;
    var mediaType = payload.mediaType;
    var boundary = payload.boundary;
    if (typeof boundary !== "number" || isNaN(boundary)) return;
    var tolerance = computeResetTolerance(mediaType);
    _mseResetBoundaries[mediaType] = {
      boundary: boundary,
      tolerance: tolerance,
    };
    _mseResetPending = true;
    logger.debug(
      "scheduling media source reinitialization at boundary " +
        boundary +
        " (" +
        mediaType +
        ", tolerance " +
    ...
    startResetSafetyTimeout();
  }

  function computeResetTolerance(mediaType) {
    var bufferManager =
      mediaType === AUDIO ? _audioBufferManager : _videoBufferManager;
    if (bufferManager) {
      var representation = bufferManager.getPlayingRepresentation();
      var frameRate = representation && representation.getFrameRate();
      if (typeof frameRate === "number" && frameRate > 0) {
        return 1 / frameRate;
      }
    }
    var fallback = _settings && _settings.mseResetTolerance;
  ...
  function startResetSafetyTimeout() {
    clearResetSafetyTimeout();
    _resetSafetyTimeout = setTimeout(function () {
      logger.error(
        "media source reinitialization gate not reached; " +
          "currentTime=" +
          _video.currentTime +
          ", readyState=" +
          _video.readyState +
          ...
          _video.paused +
          ", buffered=" +
          JSON.stringify(bufferRanges(_video.buffered)) +
          ", pendingBoundaries=" +
          JSON.stringify(_mseResetBoundaries),
      );
      _mseResetPending = false;
      _mseResetBoundaries = {};
      _video.removeAttribute("src");
      _video.load();
      var error = new Error(
        "TIMEOUT: media source reinitialization gate was not reached",
      );
  ...
  function clearResetSafetyTimeout() {
    if (_resetSafetyTimeout) {
      clearTimeout(_resetSafetyTimeout);
      _resetSafetyTimeout = null;
    }
  }

  function checkResetGate() {
    if (!_mseResetPending || _reinitializing) return;
    var currentTime = _video.currentTime;
    var boundary = 0;
    for (var mediaType in _mseResetBoundaries) {
      var entry = _mseResetBoundaries[mediaType];
      if (currentTime < entry.boundary - entry.tolerance) {
        return;
      }
    ...
    logger.debug("reset gate reached at " + currentTime);
    clearResetSafetyTimeout();
    _mseResetPending = false;
    _mseResetBoundaries = {};
    _resetBoundary = boundary;
  ...
  function reinitializeMediaSource() {
    ...
    var seekTime = _resetBoundary != null ? _resetBoundary : _video.currentTime;
    _resetBoundary = null;
...
function BufferManager(manifests, mediaSource, video, options) {
  ...
  function appendQueuedBuffers() {
    ...
    if (!_useChangeType && isCodecChange) {
      ...
      var boundary = bufferInfo.segment.startTime;
      logger.debug(
      ...
      _isBuffering = false;
      _isBufferingSegment = false;
      _resumeSegment = bufferInfo.segment.getNumber();
  ...
  function fetchSegment(url) {
    ...
    return new Promise(function (resolve) {
      ...
      xhr.addEventListener("error", function (error) {
        logger.error("segment fetch error: " + url);
        dispatchVideoErrorEvent(error);
      });
      xhr.addEventListener("timeout", function (error) {
        logger.error(
          "segment fetch timeout (" + timeoutMillis + " ms): " + url,
        );
        dispatchVideoErrorEvent(error);
      });
    });
  ...
  function setMediaSource(mediaSource) {
    _mediaSource = mediaSource;
    _sourceBuffer = null;
    if (_resumeSegment != null) {
      logger.debug("resuming buffering from segment " + _resumeSegment);
      setBufferingSegment(_resumeSegment);
      _resumeSegment = null;
      _isBuffering = false;
      _isBufferingSegment = false;
      _isAppendingBuffer = false;
